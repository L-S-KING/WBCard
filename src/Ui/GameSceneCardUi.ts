class GameSceneCardUi extends BaseUi
{
    private cardGroup_hold:eui.Image;
    private cardGroup_discard:eui.Image;
    private label_holdCard:eui.Label;
    private label_discardCard:eui.Label;
    private button_round:eui.Button;
    private pp_img:eui.Image;
    private label_pp:eui.Label;
    private top_group:eui.Group;
    private main_group:eui.Group;

    private holdCardCount:number = 0;           //牌组卡牌数量
    private discardCount:number = 0;            //弃牌堆卡牌数量

    private group_holdCard:eui.Group;
    private group_discard:eui.Group;
    
    private drawCardTimer:egret.Timer = null;    //抽取卡牌计时器

    private isBeginDrawCard:boolean = false;    //是否是第一回合抽牌

    private skipStart:boolean=false;

    private extraDraw:boolean = false;

    public constructor()
    {
        super();
        this.name = egret.getQualifiedClassName(GameSceneCardUi);
        this.skinName = "GameSceneCardUiSkin";
    }
    ...
    /**更新PP显示 */
    public updatePPLabel(pp:number)
    {
    ...
    private handCardCount:number = 0;
    private offsetX:number = 3;
    private offsetY:number = 10;

    /**根据count抽取相应数量的卡牌 */
    public drawCardByCount(count?:number)
    {
    ...
    /**更新手牌位置 */
    public updateHandCard(count:number = 0)
    {
    ...
    recvMsg(cmd:number, data:any)
    {
        switch(cmd)
        {
            ...
            this.bgRect = null;
            this.updateHandCard();
            for(let i=0;i<CardManager.Instance.selectCard.length;i++)
            {
            ...
            for(let i=0;i<CardManager.Instance.handCard.length;i++)
            {
